...
1. **그리드 시스템 (Grid System)**: ✅ 아이소메트릭 타일 기반 공장 레이아웃 (좌표 변환 함수 완료, 렌더링 구현 필요)
2. **React-Phaser 통합**: ✅ GameCanvas 컴포넌트로 게임 렌더링 (Day 2 완료)
3. **마우스 상호작용**: ✅ 그리드 클릭 시 하이라이트 표시 (Day 2 완료, 아이소메트릭 좌표계 적용 필요)
4. **e2e 테스트**: ✅ Playwright로 브라우저 환경 검증 + Agent 모드 최적화 완료 (Day 2 완료)
5. **다크/라이트 테마**: ✅ Tailwind CSS v4 기반 테마 토글 시스템 (Day 3 추가)
6. **아이소메트릭 렌더링**: 🚧 MainScene 직교 그리드를 아이소메트릭으로 변환 (Day 3 진행 중)
7. **생산 시스템 (Production System)**: 🚧 레시피 기반 케이크 제조 (Day 6 예정)
8. **물류 시스템 (Logistics System)**: 🚧 컨베이어 벨트 및 아이템 이동 (Day 7 예정)
9. **EIN/ECN 프로세스**: 🚧 엔지니어링 변경 관리 시뮬레이션 (향후 구현)
10. **시장 시스템 (Market System)**: 🚧 실시간 가격 변동 및 수요 (향후 구현)
