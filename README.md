...
- ✅ **그리드 시스템**: 20x15 그리드 기반 레이아웃 + 아이소메트릭 좌표 변환 함수
- ✅ **다크/라이트 테마**: Tailwind CSS v4 기반 테마 토글 시스템
- ✅ **마우스 상호작용**: 그리드 클릭 시 하이라이트 표시 (직교 좌표계)
- ✅ **TypeScript Project References**: 빌드 없이 실시간 개발
- ✅ **Playwright e2e 테스트**: 브라우저 환경에서 게임 동작 검증 + Agent 모드 최적화
- 🚧 **아이소메트릭 렌더링**: MainScene 직교 그리드 → 아이소메트릭 렌더링 변환 (Day 3 진행 중)
- 🚧 **아이소메트릭 마우스 처리**: 클릭 좌표를 아이소메트릭 그리드로 변환 (Day 3 진행 중)
- 📋 **설비 배치 시스템**: 개발 예정 (Day 5)
- � **생산 시스템**: 개발 예정 (Day 6)
